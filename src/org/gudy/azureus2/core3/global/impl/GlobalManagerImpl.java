	...
	implements 	GlobalManager, AEDiagnosticsEvidenceGenerator
{
	...
	private final ListenerManager<Object> listeners_and_event_listeners = 
	...
	private final ListenerManager	removalListeners 	= ListenerManager.createManager(
		"GM:DLWBRMListenDispatcher",
		new ListenerManagerDispatcherWithException() {
			public void dispatchWithException(
				Object		_listener,
				int			type,
				Object		value )
	...
	private volatile List<DownloadManager> 		managersCow	= new ArrayList<DownloadManager>();
	private final AEMonitor						managersMon		= new AEMonitor("GM:Managers");

	final Map		managerMap			= new HashMap();

	private final GlobalMangerProgressListener	progressListener;
	...
	private GlobalManagerStatsWriter 	statsWriter;
	private GlobalManagerHostSupport	hostSupport;

	private Object						downloadHistoryManager;

	// for non-persistent downloads
	private final Map<HashWrapper,Map>	savedDownloadManagerState	= new HashMap<HashWrapper,Map>();


	private int							next_seed_piece_recheck_index;

	private final TorrentFolderWatcher torrentFolderWatcher;

	private final ArrayList<Object[]> pausedList = new ArrayList<Object[]>();

	private final AEMonitor pausedListMon = new AEMonitor("GlobalManager:PL");

	private final GlobalManagerFileMerger	fileMerger;
	...
	private boolean seedingOnlyMode 			= false;
	private boolean potentiallySeedingOnlyMode	= false;
	...
	private int 	natStatus			= ConnectionManager.NAT_UNKNOWN;
	private long	natStatusLastGood	= -1;
	private boolean	natStatusProbablyOk;
				...
				try {
					...
					if ((loopFactor % saveResumeLoopCount == 0)) {
						...
						if (needsSavingCozStateChanged > 0) {
							int num_downloads = managersCow.size();
					...
					if (loopFactor % saveResumeLoopCount == 0) {
						for (Iterator it=managersCow.iterator();it.hasNext();) {
					...
					if (no_space_dl_restart_check_period_millis > 0) {
						...
						if (loopFactor % lc == 0) {
							...
							for (Iterator<DownloadManager> it=managersCow.iterator();it.hasNext();) {
		...
		long 							existingTorrentLoadDelay)
	{
		...
		try {
			statsWriter = new GlobalManagerStatsWriter(core, stats);
		} catch (Throwable e) {
		...
		try {
			try {
				...
				downloadHistoryManager = impl_class.newInstance();
		...
		if (progressListener != null) {
			progressListener.reportCurrentTask(MessageText.getString("splash.initializeGM"));
		}
		...
		trackerScraper.setClientResolver(
			new TRTrackerScraperClientResolver() {
				public boolean isScrapable(HashWrapper	torrentHash) {
					DownloadManager	dm = getDownloadManager(torrentHash);
					if (dm == null) {
						return (false);
					}
			...
			new TRTrackerScraperListener() {
				public void scrapeReceived(TRTrackerScraperResponse response) {
					HashWrapper	hash = response.getHash();
						DownloadManager manager = (DownloadManager)managerMap.get(hash);
						if (manager != null) {
		...
		try {
			hostSupport = new GlobalManagerHostSupport(this);
		} catch (Throwable e) {
			Logger.log(new LogEvent(LOGID, "Hosting unavailable", e));
		}
		checker = new Checker();
		checker.start();
		torrentFolderWatcher = new TorrentFolderWatcher(this);
		...
		fileMerger = new GlobalManagerFileMerger(this);
	}
	...
	public DownloadManager addDownloadManager(
		String 		fileName,
		byte[]	optionalHash,
		String 		savePath,
		int		 initialState,
	...
	public DownloadManager addDownloadManager(
		...
		if (!persistent) {
			Map saveDownloadState = (Map) savedDownloadManagerState.get(new HashWrapper(
					optionalHash));
			if (saveDownloadState != null) {
				if (saveDownloadState.containsKey("state")) {
					int saved_state = ((Long) saveDownloadState.get("state")).intValue();
					if (saved_state == DownloadManager.STATE_STOPPED) {
						initialState = saved_state;
					}
				}
				file_priorities = (List) saveDownloadState.get("file_priorities");
				...
				Long lPosition = (Long) saveDownloadState.get("position");
				if (lPosition != null) {
		...
		try {
			...
			// now do the creation!
			DownloadManager newManager = DownloadManagerFactory.create(this,
					optionalHash, fName, savePath, saveFile, initialState, persistent, for_seeding,
					file_priorities, adapter);
			manager = addDownloadManager(newManager, true, true);
			...
			if (manager == null || manager != newManager) {
				...
				if (initialState == DownloadManager.STATE_STOPPED) {
					if (COConfigurationManager.getBooleanParameter("Default Start Torrents Stopped Auto Pause")) {
					 	try {
								pausedListMon.enter();
								pausedList.add(new Object[]{ manager.getTorrent().getHashWrapper(), false });
						} finally {
							pausedListMon.exit();
						}
					}
	...
	public void clearNonPersistentDownloadState(byte[] hash) {
		savedDownloadManagerState.remove(new HashWrapper(hash));
	}

	protected DownloadManager addDownloadManager(
		...
		if (!isStopping) {
			...
			try {
					managersMon.enter();
					int	existing_index = managersCow.indexOf(download_manager);
				if (existing_index != -1) {
					DownloadManager existing = managersCow.get(existing_index);
				...
				Map	save_download_state	= (Map)savedDownloadManagerState.remove(hashwrapper);
				...
				if (download_manager.getPosition() == -1) {
					int endPosition = 0;
					for (int i = 0; i < managersCow.size(); i++) {
						DownloadManager dm = managersCow.get(i);
					...
					download_manager.requestAssumedCompleteMode();
					List<DownloadManager>	new_download_managers = new ArrayList<DownloadManager>(managersCow);
					new_download_managers.add(download_manager);
					managersCow	= new_download_managers;
				TOTorrent	torrent = download_manager.getTorrent();
				if (torrent != null) {
					try {
						managerMap.put(new HashWrapper(torrent.getHash()), download_manager);
				...
				if (notifyListeners) {
					...
					if (hostSupport != null) {
						hostSupport.torrentAdded( download_manager.getTorrentFileName(), download_manager.getTorrent());
					}
				}
	...
	{
		return (DownloadManager)managerMap.get(hw);
	}
	...
	{
		try {
			removalListeners.dispatchWithException(LDT_MANAGER_WBR, new Object[] {
				manager,
		...
		throws GlobalManagerDownloadRemovalVetoException
	{
		...
		if (!managersCow.contains( manager)) {
	...
	try {
		...
		List new_download_managers	= new ArrayList(managersCow);
		...
		if (torrent != null) {
			...
			try {
				managerMap.remove(new HashWrapper(torrent.getHash()));
	...
	if (hostSupport != null) {
		...
		hostSupport.torrentRemoved( manager.getTorrentFileName(), manager.getTorrent());
	}
	...
	*/
	...
	stopGlobalManager() {
		...
		try {
			...
			managersCow	= new ArrayList<DownloadManager>();
	...
	if (for_close) {
		if (progressListener != null) {
				progressListener.reportCurrentTask(MessageText.getString("splash.unloadingTorrents"));
		}
	}
	...
	for ( int i=0;i<nbDownloads;i++) {
		...
		if (progressListener != null &&	now - lastListenerUpdate > 100) {
			lastListenerUpdate = now;

			int	currentDownload = i+1;

			progressListener.reportPercent(100 * currentDownload / nbDownloads);
			progressListener.reportCurrentTask(MessageText.getString("splash.unloadingTorrent")
					+ " " + currentDownload + " "
	...
	*/
	...
	for (Iterator iter = managersCow.iterator(); iter.hasNext();) {
		...
		if (	state != DownloadManager.STATE_STOPPED &&
			...
			try {
				...
				try {
					...
					pausedList.add(new Object[]{wrapper, Boolean.valueOf(forced)});
		...
		try {
			boolean	forced = manager.isForceStart();
				...
				try {
					...
					pausedList.add(new Object[]{ manager.getTorrent().getHashWrapper(), Boolean.valueOf(forced)});
	...
	public boolean isPaused(
		...
		try {
			...
			for (int i=0; i < pausedList.size(); i++) {
					...
					Object[]	data = (Object[])pausedList.get(i);
	...
	public boolean canPauseDownloads() {
		for (Iterator i = managersCow.iterator(); i.hasNext();) {
	...
	resumeDownloads()
	{
		...
		try {
			...
			ArrayList<Object[]> copy = new ArrayList<Object[]>(pausedList);
	...
	public String isSwarmMerging(DownloadManager dm) {
		return ( fileMerger.isSwarmMerging(dm));
	}

		private List<DownloadManager>
		sortForStop() {
			List<DownloadManager>	managers = new ArrayList<DownloadManager>(managersCow);
	...
	private void loadDownloads() {
		...
		try {
			...
			try {
				...
				try {
					if (progressListener != null) {
						progressListener.reportCurrentTask(MessageText.getString("splash.loadingTorrents"));
					}
					...
					// so fix them up.
					fixUpDownloadManagerPositions();
					Logger.log(new LogEvent(LOGID, "Loaded " + managersCow.size()
							+ " torrents"));
	...
	//	if (Boolean.getBoolean("debug")) return;
		...
		try {
			...
			List<DownloadManager>	managers_temp = new ArrayList<DownloadManager>(managersCow);
		...
		managersCow = managers_temp;

		if (Logger.isEnabled())
				Logger.log(new LogEvent(LOGID, "Saving Download List ("
						+ managersCow.size() + " items)"));
		Map map = new HashMap();
		List list = new ArrayList(managersCow.size());
		for (int i = 0; i < managersCow.size(); i++) {
			DownloadManager dm = managersCow.get(i);
		...
		Map		map )
	{
		...
		if (dm != null) {
			...
			if (hostSupport != null) {
				...
				hostSupport.torrentAdded( dm.getTorrentFileName(), dm.getTorrent());
			}
		}
	...
	*/
	...
	if (managersCow != null && manager != null) {
		return managersCow.contains(manager);
	}
	return false;
	}
	...
	if (managersCow != null && manager != null)
		return managersCow.indexOf(manager);
	return -1;
	}
	...
	public void moveEnd(DownloadManager[] manager) {
		...
		for (int j = 0; j < managersCow.size(); j++) {
			DownloadManager dm = (DownloadManager) managersCow.get(j);
	...
	if (newPosition < 1 || newPosition > downloadManagerCount(curCompleted))
		...
		if (newPosition > curPosition) {
			...
			for (int i = 0; i < managersCow.size(); i++) {
			DownloadManager dm = (DownloadManager) managersCow.get(i);
	...
	public void addDownloadWillBeRemovedListener(
		...
		removalListeners.addListener(l);
	}

	public void removeDownloadWillBeRemovedListener(
		GlobalManagerDownloadWillBeRemovedListener	l) {
		removalListeners.removeListener(l);
	}
		...
		boolean		potentially_seeding )
	{
		synchronized(this) {
			...
			if (	seeding 			!= seedingOnlyMode ||
				potentially_seeding != potentiallySeedingOnlyMode) {

				seedingOnlyMode 			= seeding;
				potentiallySeedingOnlyMode	= potentially_seeding;

				// System.out.println("dispatching " + seeding_only_mode + "/" + potentially_seeding_only_mode);

				listeners_and_event_listeners.dispatch( LDT_SEEDING_ONLY, new boolean[]{ seedingOnlyMode, potentiallySeedingOnlyMode });
			}
		}
	}
	...
	isSeedingOnly()
	{
		return (seedingOnlyMode);
	}

	public boolean
	isPotentiallySeedingOnly()
	{
		return (potentiallySeedingOnlyMode);
	}
	...
	protected void computeNATStatus() {
		...
		if (num_ok > 0) {
			...
			natStatus = ConnectionManager.NAT_OK;

			natStatusLastGood = now;

		} else if (natStatusLastGood != -1 && now - natStatusLastGood < 30*60*1000) {

			natStatus = ConnectionManager.NAT_OK;

		} else if (natStatusLastGood != -1 && SystemTime.getCurrentTime() - TCPNetworkManager.getSingleton().getLastIncomingNonLocalConnectionTime() < 30*60*1000) {

			natStatus = ConnectionManager.NAT_OK;

		} else if (num_probably_ok > 0 || natStatusProbablyOk) {

			natStatus 				= ConnectionManager.NAT_PROBABLY_OK;

			natStatusProbablyOk	= true;

		} else if (num_bad > 0) {

			natStatus = ConnectionManager.NAT_BAD;

		} else {

			natStatus = ConnectionManager.NAT_UNKNOWN;
		}
	}
	...
	protected boolean seedPieceRecheck(
		DownloadManager	manager) {
	...
	public void addDownloadManagerInitialisationAdapter(
		DownloadManagerInitialisationAdapter	adapter) {
		dm_adapters.add(adapter);
	}

	public void removeDownloadManagerInitialisationAdapter(
		DownloadManagerInitialisationAdapter	adapter) {
	...
	public void generate(
		...
		try {
			...
			writer.println("	managers: " + managersCow.size());

			for (int i=0;i<managersCow.size();i++) {

				DownloadManager	manager = (DownloadManager)managersCow.get(i);
	...
	public void statsRequest(
		...
		try {
			...
			glob.put("nat", new Long( natStatus));
