	...
	extends TRTrackerServerProcessor
{
	...
	protected TRTrackerServerProcessorUDP(
	...
	public void runSupport() {
		...
		try {
			...
			if (reply != null) {
				...
				DatagramPacket replyPacket = new DatagramPacket(outputBuffer, outputBuffer.length, address, request_dg.getPort());
				socket.send(replyPacket);
				server.updateStats(request_type, torrent, input_buffer.length, outputBuffer.length);
			}
		} catch (Throwable e) {
			Logger.log(new LogEvent(LOGID,
	...
	protected long allocateConnectionId(
	...
	protected boolean checkConnectionId(
		String	client_address,
		long	id) {
		try {
			random_mon.enter();
	...
	protected PRUDPPacket handleConnect(
