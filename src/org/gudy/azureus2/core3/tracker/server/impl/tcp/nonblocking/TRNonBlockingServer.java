	...
	implements 	VirtualServerChannelSelector.SelectListener
{
	...
	*/
		...
		throws TRTrackerServerException
	{
		...
		readSelector	 	= new VirtualChannelSelector(_name + ":" + _port, VirtualChannelSelector.OP_READ, false);
		...
		SocketChannel 		channel )
    {
        ...
        if (	MAX_CONCURRENT_CONNECTIONS != 0 &&
	        			...
	        			try {
		        			...
		        			if (read_result == 0) {  //read processing is complete
			        			...
			        			if (!selector_registered) {
			        		        ...
			        		        readSelector.register(sc, this, null);
			        			} else {
			        				readSelector.resumeSelects(sc);
			        			}
		        			}
			...
			read_listener.selectSuccess(readSelector, channel, null);
        }
    }
	            		...
	            		try {
		            		...
		            		if (write_result > 0) { //more writing is needed
		            			...
		            			if (processor.getKeepAlive()) {
		            				processor.getReadListener().selectSuccess(readSelector, sc, null);
		            			} else {
		            				removeAndCloseConnection(processor);
		            			}
    	...
    	TRNonBlockingServerProcessor	processor )
    {
        ...
        try {
        	...
        	if (processors.remove( processor)) {
        		...
        		readSelector.cancel(processor.getSocketChannel());
