...
AEMonSem
{
	...
	check(
		...
		synchronized(AEMonSem.class) {
			...
			while (it.hasNext()) {
				...
				long	diff = monitor.entryCount - monitor.lastEntryCount;
				if (diff != 0) {
					active.add(monitor);
			...
			while (it.hasNext()) {
				...
				long	diff = semaphore.entryCount - semaphore.lastEntryCount;
				if (diff != 0) {
		...
		for (int i=0;i<x.length;i++) {
			AEMonSem	ms = x[i];
			long	diff = ms.entryCount - ms.lastEntryCount;
			if (current == null) {
				current	= ms;
			} else {
		...
		if (waiting_monitors.size() > 0) {
			...
			for (int i=0;i<waiting_monitors.size();i++) {
				...
				Thread last_waiter = ((AEMonitor)ms).lastWaiter;
				diag_logger.log("        [" + (last_waiter==null?"<waiter lost>":last_waiter.getName()) + "] " +  ms.name + " - " + ms.lastTraceKey);
			}
		}
		if (busy_monitors.size() > 0) {
			...
			for (int i=0;i<busy_monitors.size();i++) {
				...
				diag_logger.log("        [" + (owner==null?"<owner lost>":owner.getName()) + "] " + ms.name + " - " + ms.lastTraceKey);
			}
		}
		if (waiting_semaphores.size() > 0) {
			...
			for (int i=0;i<waiting_semaphores.size();i++) {
				...
				Thread last_waiter = ((AESemaphore)ms).latestWaiter;
				diag_logger.log("        [" + (last_waiter==null?"<waiter lost>":last_waiter.getName()) + "] " +  ms.name + " - " + ms.lastTraceKey);
			}
		}
		for (int i=0;i<x.length;i++) {
			AEMonSem	ms = x[i];
			ms.lastEntryCount = ms.entryCount;
		}
	}


	protected long			entryCount;
	protected long			lastEntryCount;
	protected String		lastTraceKey;
		...
		try {
			...
			if (!stack.isEmpty()) {
				...
				*/
				...
				boolean	check_recursion = isMonitor && !debug_recursions.contains(name);
	...
	public void trace(boolean _on) {
		trace = _on;
	}

	protected void traceEntry() {
