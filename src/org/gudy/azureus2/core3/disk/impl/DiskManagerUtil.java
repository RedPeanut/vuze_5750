...
DiskManagerUtil
{
	...
	public static void doFileExistenceChecks(
		...
		for (int i = 0; i< files.length;i++) {
			...
			if ((windowStart <= firstPiece && firstPiece <= windowEnd) || (windowStart <= lastPiece && lastPiece <= windowEnd)) { // file falls in current scanning window, check it
	...
	public static String convertDMStorageTypeToString(
		int		dm_type) {
		switch(dm_type) {
	...
	public static String convertCacheStorageTypeToString(
		int		cache_type) {
		switch(cache_type) {
