...
public class PlatformManagerImpl implements PlatformManager, AEDiagnosticsEvidenceGenerator {
	...
	private boolean		preventComputerSleepPending	= false;
	private boolean 	preventComputerSleep			= false;
	private Process		preventComputerProc;
  ...
  private boolean hasVMOptions() {
		...
		throws PlatformManagerException
	{
		synchronized(this) {
			...
			if (preventComputerSleepPending) {

				preventComputerSleepPending = false;
		...
		throws PlatformManagerException
	{
		synchronized(this) {
			if (azureusCore == null) {
				preventComputerSleepPending = prevent_it;
				return;
			}
			if (preventComputerSleep == prevent_it) {
				return;
			}
			preventComputerSleep = prevent_it;
			if (prevent_it) {
				String[] command;
				...
				if (preventComputerProc != null) {
					Debug.out("eh?");
					preventComputerProc.destroy();
				}
				try {
					System.out.println("Starting idle sleep preventer: " + command[0]);
					preventComputerProc = Runtime.getRuntime().exec(command);
				} catch (Throwable e) {
					Debug.out(e);
				}
			} else {
	...
	public boolean isAdditionalFileTypeRegistered(
	...
	public void unregisterAdditionalFileType(
		String		name,				// e.g. "BitTorrent"
		String		type )				// e.g. ".torrent"
	...
	public void registerAdditionalFileType(
		String		name,				// e.g. "BitTorrent"
	...
	public void copyFilePermissions(
	...
	public boolean testNativeAvailability(
		String	name )
