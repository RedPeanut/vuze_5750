	...
	implements IMainStatusBar, UIUpdatableAlways
{
	...
	private Composite pluginLabelComposite;
	...
	public MainStatusBar() {
		...
		AzureusCoreFactory.addCoreRunningListener(new AzureusCoreRunningListener() {
			public void azureusCoreRunning(AzureusCore core) {
				...
				connectionManager = PluginInitializer.getDefaultInterface().getConnectionManager();
				PluginInterface dhtPI = pm.getPluginInterfaceByClass(DHTPlugin.class);
				if (dhtPI != null) {
					dhtPlugin = (DHTPlugin) dhtPI.getPlugin();
				}
			}
		});
	}
	...
	public Composite initStatusBar(final Composite _parent) {
		...
		this.pluginLabelComposite = new Composite(statusBar, SWT.NONE);
		this.pluginLabelComposite.setForeground(fgColor);
		...
		pluginLabelComposite.setLayout(gridLayout);
		pluginLabelComposite.setLayoutData(gridData);
	...
	public void updateUI(boolean is_visible) {
		...
		// Plugins.
		Control[] pluginElements = this.pluginLabelComposite.getChildren();
		for (int i = 0; i < pluginElements.length; i++) {
			if (pluginElements[i] instanceof UpdateableCLabel) {
				((UpdateableCLabel) pluginElements[i]).checkForRefresh();
			}
		}
		...
		if (AzureusCoreFactory.isCoreRunning()) {
			...
			if (NetworkManager.isSeedingOnlyUploadRate()) {
				...
				if (ul_limit_seed == 0) {
					seedingOnly = "+" + Constants.INFINITY_STRING + "K";
				} else {
					int diff = ul_limit_seed - ul_limit_norm;
					seedingOnly = (diff >= 0 ? "+" : "") + diff + "K";
				}
			} else {
				seedingOnly = "";
			}
	...
	private void updateDHTStatus() {
		...
		// DHT Status Section
		int _dhtStatus = dhtPlugin.getStatus();
		long _dhtCount = -1;
		//boolean reachable = false;
		if (_dhtStatus == DHTPlugin.STATUS_RUNNING) {
			DHT[] dhts = dhtPlugin.getDHTs();

			//reachable = dhts.length > 0 && dhts[0].getTransport().isReachable();
			//if (reachable) {
			_dhtCount = dhts[0].getControl().getStats().getEstimatedDHTSize();
			//}
		}

		if (lastDHTstatus != _dhtStatus || lastDHTcount != _dhtCount) {
			...
			switch (_dhtStatus) {
				case DHTPlugin.STATUS_RUNNING:
					...
					dhtStatus.setText(MessageText.getString("MainWindow.dht.status.users").replaceAll(
							"%1", numberFormat.format(_dhtCount)));
			...
			lastDHTstatus = _dhtStatus;
			lastDHTcount = _dhtCount;
		}
	}
	...
	private void updateNatStatus() {
		...
		int nat_status = connectionManager.getNATStatus();
	...
	public void createStatusEntry(final CLabelUpdater updater) {
		AERunnable r = new AERunnable() {
			public void runSupport() {
				UpdateableCLabel result = new UpdateableCLabel(pluginLabelComposite, borderFlag,
						updater);
	...
	private void layoutPluginComposite() {
		...
		for (int i = 0; i < pluginElements.length; i++) {
			if (pluginElements[i] instanceof UpdateableCLabel) {
				((UpdateableCLabel) pluginElements[i]).reset();
			}
		}
		statusBar.layout();
	}
		...
		implements IProgressReportingListener
	{
		...
		private void swt_updateProgressBarDisplay(
