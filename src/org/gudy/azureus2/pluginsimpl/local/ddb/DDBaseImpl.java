	...
	implements DistributedDatabase
{
		...
		getDHT().registerHandler(
			...
			new DHTPluginTransferHandler() {
				...
				public byte[] handleRead(
					...
					try {
						DDBaseValueImpl	res = (DDBaseValueImpl) handler.read(
				...
				public byte[] handleWrite(
					...
					try {
						...
						DDBaseValueImpl	res = (DDBaseValueImpl) handler.write(
								contact,
								type,
								new DDBaseKeyImpl(xfer_key),
	...
	protected DistributedDatabaseValue read(
