	...
	implements PluginConfig
{
	...
	static {
		...
		String[] passthroughParams = new String[] {
				"Open MyTorrents", "IconBar.enabled", "Wizard Completed",
				"welcome.version.lastshown",
		};

		for (int i=0; i<passthroughParams.length; i++) {
			external_to_internal_key_map.put(passthroughParams[i], passthroughParams[i]);
		}
	}
	...
	private static Object getFakeValueWhenDisabled(
		String	key,
		String	name) {
	...
	private boolean getDefaultedBooleanParameter(String name, boolean map_name) {
		...
		return getBooleanParameter(name, ConfigurationDefaults.defBoolean == 1, map_name, false);
	}

	private byte[] getDefaultedByteParameter(String name, boolean map_name) {
		return getByteParameter(name, ConfigurationDefaults.defBytes, map_name, false);
	}

	private int[] getDefaultedColorParameter(String name, boolean map_name) {
		int[] default_value = new int[] {
				ConfigurationDefaults.defInt,
				ConfigurationDefaults.defInt,
				ConfigurationDefaults.defInt,
				1
		};
	...
	private float getDefaultedFloatParameter(String name, boolean map_name) {
		return getFloatParameter(name, ConfigurationDefaults.defFloat, map_name, false);
	}

	private int getDefaultedIntParameter(String name, boolean map_name) {
		return getIntParameter(name, ConfigurationDefaults.defInt, map_name, false);
	}

	private long getDefaultedLongParameter(String name, boolean map_name) {
		return getLongParameter(name, ConfigurationDefaults.defLong, map_name, false);
	}

	private String getDefaultedStringParameter(String name, boolean map_name) {
		return getStringParameter(name, ConfigurationDefaults.defString, map_name, false);
	}
