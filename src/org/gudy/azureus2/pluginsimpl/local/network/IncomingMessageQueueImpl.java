...
public class IncomingMessageQueueImpl implements IncomingMessageQueue {
	private final com.aelitis.azureus.core.networkmanager.IncomingMessageQueue coreQueue;
	...
	private void registerListenerSupport(final IncomingMessageQueueListener listener, final boolean isPriority) {
		com.aelitis.azureus.core.networkmanager.IncomingMessageQueue.MessageQueueListener coreListener =
			new com.aelitis.azureus.core.networkmanager.IncomingMessageQueue.MessageQueueListener() {
				...
				public void protocolBytesReceived(int byteCount) {	
					listener.bytesReceived(byteCount);	
				}
		...
		registrations.put(listener, coreListener);	//save this mapping for later
		coreQueue.registerQueueListener(coreListener);
	}
	...
	if (core_listener != null) {
		coreQueue.cancelQueueListener(core_listener);
	}
	}
	...
	if (message instanceof MessageAdapter) {
		...
		coreQueue.notifyOfExternallyReceivedMessage(((MessageAdapter)message).getCoreMessage());
		return;
	}
	//message originally created by plugin
	coreQueue.notifyOfExternallyReceivedMessage(new MessageAdapter( message ));
	}
	public int getPercentDoneOfCurrentMessage() {
	return coreQueue.getPercentDoneOfCurrentMessage();
	}
}
