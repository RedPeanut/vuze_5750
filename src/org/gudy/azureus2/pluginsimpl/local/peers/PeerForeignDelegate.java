	...
	implements 	PEPeerTransport
{
	...
	private NetworkConnectionBase	networkConnection;
	...
	protected PeerForeignDelegate(
		...
		networkConnection = new PeerForeignNetworkConnection(this, foreign);

		networkConnection.addRateLimiter(pm.getUploadLimitedRateGroup(), true);
		networkConnection.addRateLimiter(pm.getDownloadLimitedRateGroup(), false);
		...
		_foreign.bindConnection(
			new ConnectionStub() {
				  ...
				  addRateLimiter(
					  ...
					  boolean			is_upload )
				  {
					  networkConnection.addRateLimiter(UtilitiesImpl.wrapLimiter( limiter, false ), is_upload);
				  }
				  ...
				  removeRateLimiter(
					  ...
					  boolean			is_upload )
				  {
					  networkConnection.removeRateLimiter(UtilitiesImpl.wrapLimiter( limiter, false ), is_upload);
				  }
				  ...
				  getRateLimiters(
					 boolean			is_upload )
				  {
						LimitedRateGroup[] limiters = networkConnection.getRateLimiters(is_upload);
	...
	public void start() {
		NetworkManager.getSingleton().startTransferProcessing(networkConnection);
		NetworkManager.getSingleton().upgradeTransferProcessing(networkConnection, manager.getPartitionID());
	}

	protected void stop() {
		NetworkManager.getSingleton().stopTransferProcessing(networkConnection);
	}
   ...
   */
	...
	public boolean requestAllocationStarts(
		int[]	base_priorities) {
  ...
  public void setOptimisticUnchoke(boolean is_optimistic) {
	...
	public void setUploadRateLimitBytesPerSecond(int bytes) { networkConnection.setUploadLimit( bytes); }
	public void setDownloadRateLimitBytesPerSecond(int bytes) { networkConnection.setDownloadLimit( bytes); }
	public int getUploadRateLimitBytesPerSecond() { return networkConnection.getUploadLimit(); }
	public int getDownloadRateLimitBytesPerSecond() { return networkConnection.getDownloadLimit(); }
	...
	public void addRateLimiter(
		...
		boolean				upload) {
		networkConnection.addRateLimiter(limiter, upload);
	}

	public LimitedRateGroup[]
	getRateLimiters(
		boolean	upload) {
		return (networkConnection.getRateLimiters( upload));
	}

	public void removeRateLimiter(
		LimitedRateGroup	limiter,
		boolean				upload) {
		networkConnection.removeRateLimiter(limiter, upload);
	}
	...
	public void setHaveAggregationEnabled(
		boolean		enabled) {
	}
