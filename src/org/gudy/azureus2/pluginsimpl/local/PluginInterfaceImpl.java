...
public final class PluginInterfaceImpl
	...
	private Object				initialiserKey;
	protected ClassLoader		classLoader;

	private CopyOnWriteList<PluginListener>		listeners 				= new CopyOnWriteList<PluginListener>();
	private Set<PluginListener>					initCompleteFiredSet	= new HashSet<PluginListener>();

	private CopyOnWriteList<PluginEventListener>		eventListeners	= new CopyOnWriteList<PluginEventListener>();
	private String				key;
	...
	private IPCInterfaceImpl	ipcInterface;
		...
		throws PluginException
	{
		...
		initialiserKey		= _initialiser_key;
		classLoader			= _class_loader;
		key					= _key;
		...
		givenPluginId			= _plugin_id;
		pluginVersion		= _plugin_version;
		ipcInterface			= new IPCInterfaceImpl(initialiser, plugin);
	...
	public boolean isOperational() {
		PluginDeprecation.call("isOperational", this.givenPluginId);
		return getPluginState().isOperational();
	}

	public Object getInitializerKey() {
		return (initialiserKey);
	}
	...
	public String getPluginID() {
		...
		if (pluginIdToUse != null) {return pluginIdToUse;}
	...
	// value we were given.
		...
		if (id == null) {id = givenPluginId;}
		if (id == null) {id = "<none>";}

		pluginIdToUse = id;
		return pluginIdToUse;
	}

	public boolean isMandatory() {
		PluginDeprecation.call("isMandatory", this.givenPluginId);
		return getPluginState().isMandatory();
	}

	public boolean isBuiltIn() {
		PluginDeprecation.call("isBuiltIn", this.givenPluginId);
	...
	public void setPluginDirectoryName(
		String		name ) {
		initialiserKey	= new File(name);
	...
	public boolean isUnloadable() {
		PluginDeprecation.call("unloadable", this.givenPluginId);
		return getPluginState().isUnloadable();
	}

	public void reload() throws PluginException {
		PluginDeprecation.call("reload", this.givenPluginId);
		getPluginState().reload();
	}

	public void unload() throws PluginException {
		PluginDeprecation.call("unload", this.givenPluginId);
		getPluginState().unload();
	}

	public void uninstall() throws PluginException {
		PluginDeprecation.call("uninstall", this.givenPluginId);
	...
	protected void fireInitComplete(PluginListener listener) {
		synchronized(initCompleteFiredSet) {
			if (initCompleteFiredSet.contains(listener)) {
				return;
			}
			initCompleteFiredSet.add(listener);
		}
		try {
	...
	public boolean isShared() {
		PluginDeprecation.call("isShared", this.givenPluginId);
	...
	protected void generateEvidence(
		IndentWriter		writer) {
	...
	public boolean isDisabled() {
		PluginDeprecation.call("isDisabled", this.givenPluginId);
		return getPluginState().isDisabled();
	}

	public void setDisabled(boolean disabled) {
		PluginDeprecation.call("setDisabled", this.givenPluginId);
