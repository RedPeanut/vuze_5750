	...
	implements SpeedManager, SpeedManagerAlgorithmProviderAdapter, AEDiagnosticsEvidenceGenerator
{
	...
	public void setEstimatedUploadCapacityBytesPerSec(
		int		bytes_per_sec,
		float	metric) {
	...
	public void setEstimatedDownloadCapacityBytesPerSec(
		int		bytes_per_sec,
		float	metric) {
	...
	protected void setEnabledSupport(
		boolean	_enabled) {
		if (enabled != _enabled) {
		...
		implements SpeedManagerAlgorithmProvider
	{
		...
		public void pingSourceFound(
			SpeedManagerPingSource		source,
			boolean						is_replacement) {
		}

		public void pingSourceFailed(
			SpeedManagerPingSource		source) {
		}
