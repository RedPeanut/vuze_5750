...
public class RelatedContentSearcher
	...
	private String TAG = RelatedContentSearcher.class.getSimpleName();
	...
	private ByteArrayHashMap<ForeignBloom>		harvestedBlooms 	= new ByteArrayHashMap<ForeignBloom>();
	...
	private final AsyncDispatcher	harvestDispatcher			= new AsyncDispatcher();

	private final RelatedContentManager					manager;
	private final DistributedDatabaseTransferType		transfer_type;
	private final DHTPluginInterface					dhtPlugin;
	...
	private void checkDDB() {
		if (ddb == null) {
			...
			try {
				List<DistributedDatabase> ddbs = DDBaseImpl.getDDBs(new String[]{ dhtPlugin.getNetwork() });
					...
					try {
						...
						try {
							...
							if (ddb != null) {
								...
								DHTInterface[]	dhts = dhtPlugin.getDHTInterfaces();

								boolean public_dht = dhtPlugin.getNetwork() == AENetworkClassifier.AT_PUBLIC;
	...
	public DistributedDatabaseValue write(
		DistributedDatabaseContact			contact,
		DistributedDatabaseTransferType		type,
	...
	private List<DownloadInfo> getDHTInfos(boolean search_cvs_only) {
		...
		/*Log.d(TAG, "getDHTInfos() is called...");
		Throwable t = new Throwable();
		t.printStackTrace();*/
		
		List<DHTPluginValue> vals;
		if (search_cvs_only) {
			if (dhtPlugin instanceof DHTPlugin) {
				vals = ((DHTPlugin)dhtPlugin).getValues(DHTPlugin.NW_CVS,  false);
			} else {
				vals = dhtPlugin.getValues();
			}
		} else {
			vals = dhtPlugin.getValues();
		}
	...
	private void harvestBlooms() {
		harvestDispatcher.dispatch(
			new AERunnable() {
				public void runSupport() {
					if (harvestDispatcher.getQueueSize() > 0) {
						return;
					}
					ForeignBloom oldest = null;
					synchronized(harvestedBlooms) {
						for (ForeignBloom bloom: harvestedBlooms.values()) {
							if (oldest == null ||
					...
					if (oldest != null) {
						if (now - oldest.getLastUpdateTime() > HARVEST_BLOOM_UPDATE_MILLIS) {
							...
							if (now - oldest.getCreateTime() > HARVEST_BLOOM_DISCARD_MILLIS &&
									harvestedBlooms.size() >= HARVEST_MAX_BLOOMS / 2) {
								...
								synchronized(harvestedBlooms) {
									harvestedBlooms.remove( ddb_contact.getID());
								}
							} else {
					...
					if (harvestedBlooms.size() < HARVEST_MAX_BLOOMS) {
						try {
							int	failCount	= 0;
							DHTInterface[] dhts = dhtPlugin.getDHTInterfaces();
outer:
							for (DHTInterface dht: dhts) {
								...
								DHTPluginContact[] contacts = dht.getReachableContacts();
								byte[] dhtId = dht.getID();
								for (DHTPluginContact contact: contacts) {
									byte[]	contact_id = contact.getID();
									if (Arrays.equals( dhtId, contact_id)) {
										// logSearch("not skipping local!!!!");
										continue;
									}
									DistributedDatabaseContact ddbContact = importContact(contact, network);
									synchronized(harvestedBlooms) {
										if (harvested_fails.containsKey( contact_id)) {
											continue;
										}
										if (harvestedBlooms.containsKey( contact_id)) {
											continue;
										}
									}
									BloomFilter filter = sendRemoteFetch(ddbContact);
									logSearch("harvest: " + contact.getString() + " -> " +(filter==null?"null":filter.getString()));
									if (filter != null) {
										synchronized(harvestedBlooms) {
											harvestedBlooms.put(contact_id, new ForeignBloom( ddbContact, filter));
										}
										break outer;
									} else {
										synchronized(harvestedBlooms) {
	...
	foreignBloomFailed(
		...
		synchronized(harvestedBlooms) {
			...
			if (harvestedBlooms.remove(contact_id) != null) {
	...
	private boolean
	isPopularity(String		term) {
		return (term.equals("(.)"));
	}

	private List<DistributedDatabaseContact>
	searchForeignBlooms(String		term) {
		...
		try {
			...
			synchronized(harvestedBlooms) {
				...
				for ( ForeignBloom fb: harvestedBlooms.values()) {
