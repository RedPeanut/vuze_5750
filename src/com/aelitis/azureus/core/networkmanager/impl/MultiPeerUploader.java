...
public class MultiPeerUploader implements RateControlledEntity {
	...
	private final RateHandler rateHandler;
	...
	private int write(EventWaiter waiter, int num_bytes_to_write, boolean protocol_is_free) {	
		...
		if (num_bytes_written > 0) {
			rateHandler.bytesProcessed(data_bytes_written, protocol_bytes_written);
		}
	...
	*/
	...
	int[] allowed = rateHandler.getCurrentNumBytesAllowed();
	...
	getString()
	{
		...
		str.append("can_process=").append(canProcess(null)).append(",bytes_allowed=").append(rateHandler.getCurrentNumBytesAllowed())
			.append(", waiting=");
		try {
			lists_lock.enter();
			int	num = 0;
