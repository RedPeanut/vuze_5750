...
IncomingConnectionManager
{
	...
	private final AEMonitor connectionsMon = new AEMonitor("IncomingConnectionManager:conns");
	...
	public void deregisterMatchBytes(
	...
	protected void doTimeoutChecks() {
		try {
			...
			for (int i=0; i < connections.size(); i++) {
				...
				if (ic.lastReadTime > 0) {	//at least one read op has occured
					if (now < ic.lastReadTime) {	//time went backwards!
						ic.lastReadTime = now;
					}
					else if (now - ic.lastReadTime > transport_helper.getReadTimeout()) {
						if (Logger.isEnabled())
		...
		implements TransportHelper.selectListener
	{
		...
		public boolean selectSuccess(TransportHelper transportHelper, Object attachment) {
			IncomingConnection	ic = (IncomingConnection)attachment;

			try {
				long bytesRead = ic.filter.read(new ByteBuffer[]{ ic.buffer }, 0, 1);
				...
				ic.lastReadTime = SystemTime.getCurrentTime();

				Object[] matchData = checkForMatch(transportHelper, localPort, ic.buffer, false);

				if (matchData == null) {	//no match found
					if (transportHelper.isClosed() ||
						...
						if (Logger.isEnabled())
							Logger.log(new LogEvent(LOGID,
									LogEvent.LT_WARNING,
									"Incoming stream from [" + transportHelper.getAddress()
									+ "] does not match "
					...
					IncomingConnectionManager.MatchListener listener = (IncomingConnectionManager.MatchListener)matchData[0];
					listener.connectionMatched(transport, matchData[1]);
				}
				return (true);
			}
			catch (Throwable t) {
				try {
					if (Logger.isEnabled())
						Logger.log(new LogEvent(LOGID,
								LogEvent.LT_WARNING,
								"Incoming connection [" + transportHelper.getAddress()
