...
public class HTTPMessageDecoder
	...
	private static String TAG = HTTPMessageDecoder.class.getSimpleName();
		...
		throws IOException
	{
		...
		if (header_ready) {
			...
			httpConnection.decodeHeader( this, header_so_far.toString());
			...
			while (rem > 0 && !( paused || paused_internally)) {
				...
				if (c == '\n') {
					...
					if (header_str.endsWith( NL + NL)) {
						httpConnection.decodeHeader(this, header_str);
						header_so_far.setLength(0);
					}
				}
			}
	...
	protected void addMessage(Message message) {
		...
		Log.d(TAG, "addMessage() is called...");
