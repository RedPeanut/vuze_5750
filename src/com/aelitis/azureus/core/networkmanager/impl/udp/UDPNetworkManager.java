...
public class UDPNetworkManager {
	...
	public static void refreshRates(int minRate) {
		udp_mss_size = COConfigurationManager.getIntParameter("network.udp.mtu.size") - 40;
	    if (udp_mss_size > minRate)  udp_mss_size = minRate - 1;
	...
	private int udpNonDataListenPort = -1;
				   ...
				   new ParameterListener() {
					   public void parameterChanged(String name) {
						   ...
						   if (port < 0 || port > 65535 || port == Constants.INSTANCE_PORT) {
						        ...
						        Debug.out(msg);
						        Logger.log(new LogAlert(LogAlert.UNREPEATABLE, LogAlert.AT_ERROR, msg));
						        udpNonDataListenPort = RandomUtils.generateRandomNetworkListenPort();
						        COConfigurationManager.setParameter(name, udpNonDataListenPort);
						    } else {
						    	udpNonDataListenPort	= port;
						    }
					   }
				   });
	}
