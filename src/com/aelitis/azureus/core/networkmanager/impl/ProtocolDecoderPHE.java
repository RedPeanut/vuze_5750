	...
	extends ProtocolDecoder
{
	...
	public static void addSecretsSupport(
		byte[][]		secrets) {
	...
	private long	lastReadTime	= SystemTime.getCurrentTime();
	...
	private final AEMonitor	processMon	= new AEMonitor("ProtocolDecoderPHE:process");

	private static String TAG = ProtocolDecoderPHE.class.getSimpleName();
	
	public ProtocolDecoderPHE(
		...
		throws IOException
	{
		...
		try {
			...
			transport.registerForReadSelects(
				new TransportHelper.selectListener() {
				   	public boolean selectSuccess(
						...
						Object 			attachment)
				   	{
				   		/*Log.d(TAG, "readSelects.selectSuccess() is called...");
			...
			transport.registerForWriteSelects(
					new TransportHelper.selectListener() {
					   	...
					   	public boolean selectSuccess(
							...
							Object 			attachment)
					   	{
					   		/*Log.d(TAG, "writeSelects.selectSuccess() is called...");
					   		Throwable t = new Throwable();
					   		t.printStackTrace();*/
			...
			if (outbound) {
				protocolState	= PS_OUTBOUND_1;
				transport.pauseReadSelects();
			} else {
				protocolState	= PS_INBOUND_1;
				readBuffer = ByteBuffer.allocate(dh_public_key_bytes.length);
				readBuffer.put(_header);
				bytesRead += _header.limit();
			}
		} finally {
			processMon.exit();
		}
		...
		throws IOException
	{
		try {
			...
			while (loop) {
				...
				if (protocolState == PS_OUTBOUND_1) {
					if (writeBuffer == null) {
						// A sends B Ya + Pa
						byte[]	padding_a = getRandomPadding(getPaddingMax()/2);	// note that /2 also used in calculating max initial packet size above
						writeBuffer = ByteBuffer.allocate(dh_public_key_bytes.length + padding_a.length);
						writeBuffer.put(dh_public_key_bytes);
						writeBuffer.put(padding_a);
						writeBuffer.flip();
					}
					write(writeBuffer);
					if (!writeBuffer.hasRemaining()) {
						writeBuffer	= null;
						protocolState	= PS_INBOUND_2;
					}
				} else if (protocolState == PS_INBOUND_1) {
					// B receives Ya
					read(readBuffer);
					if (!readBuffer.hasRemaining()) {
						readBuffer.flip();
						byte[] other_dh_public_key_bytes = new byte[readBuffer.remaining()];
						readBuffer.get(other_dh_public_key_bytes);
						completeDH(other_dh_public_key_bytes);
						readBuffer	= null;
						protocolState	= PS_OUTBOUND_2;
					}
				} else if (protocolState == PS_OUTBOUND_2) {
					// B->A: Yb PadB
					if (writeBuffer == null) {
						byte[]	padding_b = getRandomPadding(getPaddingMax()/2);
						writeBuffer = ByteBuffer.allocate(dh_public_key_bytes.length + padding_b.length);
						writeBuffer.put(dh_public_key_bytes);
						writeBuffer.put(padding_b);
						writeBuffer.flip();
					}
					write(writeBuffer);
					if (!writeBuffer.hasRemaining()) {
						writeBuffer	= null;
						protocolState	= PS_INBOUND_3;
					}
				} else if (protocolState == PS_INBOUND_2) {
					// A receives: Yb
					if (readBuffer == null) {
						readBuffer = ByteBuffer.allocate(dh_public_key_bytes.length);
					}
					...
					if (!readBuffer.hasRemaining()) {
						...
						setupCrypto();
						readBuffer	= null;
						protocolState	= PS_OUTBOUND_3;
					}
				} else if (protocolState == PS_OUTBOUND_3) {
					// A->B: HASH('req1', S), HASH('req2', SKEY)^HASH('req3', S), ENCRYPT(VC, crypto_provide, len(PadC), PadC, len(IA)), ENCRYPT(IA)
					if (writeBuffer == null) {
						...
						writeBuffer = ByteBuffer.allocate(padding_a.length + 20 + 20 + ( VC.length + 4 + 2 + padding_c.length + 2 ) + initial_data_out_len);
						writeBuffer.put(padding_a);
							// HASH('req1', S)
						SHA1Hasher	hasher = new SHA1Hasher();
						...
						writeBuffer.put(sha1_1);
						// ENCRYPT(VC, crypto_provide, len(PadC), PadC, len(IA)
						writeBuffer.put(write_cipher.update( VC));
						writeBuffer.put(write_cipher.update(new byte[]{ 0, 0, 0, my_supported_protocols }));
						writeBuffer.put(write_cipher.update(new byte[]{ (byte)(padding_c.length>>8),(byte)padding_c.length }));
						writeBuffer.put(write_cipher.update( padding_c));
						writeBuffer.put(write_cipher.update(new byte[]{ (byte)(initial_data_out_len>>8),(byte)initial_data_out_len }));
						if (initial_data_out_len > 0) {
							int	save_pos = initial_data_out.position();
							write_cipher.update(initial_data_out, writeBuffer);
					...
					while (true) {
						...
						if (protocolSubstate == 1) {
							 	//skip up to HASH('req1', S)
							int	limit = readBuffer.limit();
							readBuffer.position(limit - 20);
							boolean match	= true;
							for (int i=0;i<20;i++) {
								if (readBuffer.get() != padding_skip_marker[i]) {
									match	= false;
									break;
								}
							}
							if (match) {
								readBuffer = ByteBuffer.allocate(20 + VC.length + 4 + 2);
								protocolSubstate	= 2;
								break;
							} else {
								if (limit == readBuffer.capacity()) {
									throw (new IOException("PHE skip to SHA1 marker failed"));
								}
								readBuffer.limit(limit + 1);
								readBuffer.position(limit);
							}
						} else if (protocolSubstate == 2) {
							// find SKEY using HASH('req2', SKEY)^HASH('req3', S)  , ENCRYPT(VC, crypto_provide, len(PadC),
							readBuffer.flip();
							final byte[]	decode = new byte[20];
							readBuffer.get(decode);
							SHA1Hasher hasher = new SHA1Hasher();
							hasher.update(REQ3_IV);
							hasher.update(secret_bytes);
							byte[] sha1 = hasher.getDigest();
							...
							readBuffer = ByteBuffer.allocate(padding + 2);
								// skip the padding
							protocolSubstate	= 3;
						} else if (protocolSubstate == 3) {
							...
							if (ia_len > 0) {
								readBuffer = ByteBuffer.allocate(ia_len);
								// skip the padding
								protocolSubstate	= 4;
							} else {
								readBuffer	= null;
								protocolState = PS_OUTBOUND_4;
								break;
							}
						} else if (protocolSubstate == 4) {
							// ENCRYPT(IA)
							readBuffer.flip();
							byte[]	data = new byte[readBuffer.remaining()];
							readBuffer.get(data);
				...
				} else if (protocolState == PS_OUTBOUND_4) {
					// B->A: ENCRYPT(VC, crypto_select, len(padD), padD, // len(IB)), ENCRYPT(IB)
					if (writeBuffer == null) {
						int	pad_max = getPaddingMax();
						byte[]	padding_b = getRandomPadding(pad_max/2);	// half padding b sent here
						byte[]	padding_d = getZeroPadding(pad_max);
						writeBuffer = ByteBuffer.allocate(padding_b.length + VC.length + 4 + 2 + padding_d.length); // + 2 + initial_data_out.length);
						writeBuffer.put(padding_b);
						writeBuffer.put(write_cipher.update( VC));
						writeBuffer.put( write_cipher.update(new byte[]{ 0, 0, 0, selected_protocol }));
						writeBuffer.put( write_cipher.update(new byte[]{ (byte)(padding_d.length>>8),(byte)padding_d.length }));
						writeBuffer.put(write_cipher.update( padding_d));
					...
					if (delay_outbound_4) {
						if (transport.delayWrite( writeBuffer)) {
							writeBuffer	= null;
							handshakeComplete();
						} else {
				...
				} else if (protocolState == PS_INBOUND_4) {
					// B->A: ENCRYPT(VC, crypto_select, len(padD), padD // , len(IB)), ENCRYPT(IB)
					if (readBuffer == null) {
						readBuffer = ByteBuffer.allocate(VC.length + PADDING_MAX);
						readBuffer.limit(VC.length);
					...
					while (true) {
						...
						if (protocolSubstate == 1) {
						 	//skip up to marker
							int	limit = readBuffer.limit();
							readBuffer.position(limit - VC.length);
							boolean match	= true;
							...
							if (match) {
								readBuffer = ByteBuffer.allocate(4 + 2);
								protocolSubstate	= 2;
								break;
							} else {
								if (limit == readBuffer.capacity()) {
							...
							readBuffer = ByteBuffer.allocate(pad_len); // + 2);
							protocolSubstate	= 3;
						} else if (protocolSubstate == 3) {
							readBuffer.flip();
							byte[]	data = new byte[readBuffer.remaining()];
	...
	*/
	...
	public boolean selectSuccess(
		...
		try {
			int	old_bytes_read		= bytesRead;
			int	old_bytes_written	= bytesWritten;
			process();
			if (write_operation) {
				return (bytesWritten != old_bytes_written);
			} else {
				boolean	progress = bytesRead != old_bytes_read;
				if (progress) {
	...
	public String getString() {
		return ("state=" + protocolState + ",sub=" + protocolSubstate + ",in=" + bytesRead + ",out=" + bytesWritten);
	}
}
