	...
	implements ContentNetworkManager, AEDiagnosticsEvidenceGenerator
{
	...
	protected ContentNetworkImpl addNetwork(ContentNetworkImpl network) {
		...
		synchronized(this) {
			...
			while (it.hasNext()) {
				ContentNetworkImpl existingNetwork = it.next();
				if (existingNetwork.getID() == network.getID()) {
					if (network.getVersion() > existingNetwork.getVersion()) {
						try {
							existingNetwork.updateFrom(network);
						} catch (Throwable e) {
							Debug.printStackTrace(e);
						}
						...
						log("Network " + existingNetwork.getString() + " already up to date");
