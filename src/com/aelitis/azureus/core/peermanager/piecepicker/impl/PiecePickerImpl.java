...
public class PiecePickerImpl implements PiecePicker {
	...
	private boolean				reverseBlockOrder;
	private int[]				globalRequestHint;
...
//		computeBasePriorities();
		...
		for (int i =0; i <uploadersSize; i++) {
			...
			if (pt.isDownloadPossible()) {
				...
				//	System.out.println("\treqesting from peer; speed:"+pt.getStats().getDataReceiveRate()+" outstanding requests:"+pt.getNbRequests()+" max:"+maxRequests);
					...
					try {
						boolean	peerManagingRequests = pt.requestAllocationStarts(startPriorities);
						while (pt.isDownloadPossible() && pt.getNbRequests() < maxRequests) {
							// is there anything else to download?
							int	allocated;
							if (peerManagingRequests || !endGameMode) {
	...
	protected int getNextBlockETAFromNow(
			PEPeerTransport	pt) {
...
//						startPriority +=(PRIORITY_W_RARE +peerControl.getNbPeers()) /avail;
	...
	protected final int findPieceToDownload(PEPeerTransport pt, int nbWanted) {
		...
		if (enable_request_hints) {
			...
			if (request_hint == null) {
				request_hint = globalRequestHint;
		...
		final int[] blocksFound = pePiece.getAndMarkBlocks(pt, nbWanted, request_hint, reverseBlockOrder );
	...
	private final int getRequestCandidate(final PEPeerTransport pt) {
		...
		if (!forceStart || startCandidates ==null ||startCandidates.nbSet <=0) {
			...
			*/
			...
			// check at arbitrary figure of 32 pieces
			if (reservedPieceNumber >=0 &&globalMinOthers >0 &&peerControl.getNbActivePieces() >32)	{ 
	...
	public void removePriorityProvider(
		PiecePriorityProvider		provider) {
	...
	public void setGlobalRequestHint(
		int	piece_number,
		int	start_bytes,
		int	byte_count) {
		if (piece_number < 0) {

			globalRequestHint = null;

		} else {

			globalRequestHint = new int[]{ piece_number, start_bytes, byte_count };
		}
	}

	public int[]
	getGlobalRequestHint() {
		return (globalRequestHint);
	}

	public void setReverseBlockOrder(
		boolean		is_reverse) {
		reverseBlockOrder = is_reverse;
	}

	public boolean getReverseBlockOrder() {
		return (reverseBlockOrder);
	}
