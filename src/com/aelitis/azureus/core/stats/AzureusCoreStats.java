...
public class AzureusCoreStats {
	...
	private static final CopyOnWriteList providerListeners = new CopyOnWriteList();
	private static final CopyOnWriteList derived_generators = new CopyOnWriteList();

	public static void addStatsDefinitions(
		String[][]		stats) {
	...
	public static void addProvidersChangeListener(providersChangeListener l) {
		providerListeners.add(l);
	}

	protected static void fireProvidersChangeListeners() {
		Iterator it = providerListeners.iterator();
		while (it.hasNext()) {
			try {
	...
	public static void registerDerivedStatsGenerator(
