	...
	implements AZInstanceManager, MCGroupAdapter
{
	...
	protected AZInstanceManagerImpl(
	...
	protected void modifyAddresses(
	...
	public InetSocketAddress getLANAddress(
		...
		Map	map;
		if (addressType == AT_TCP) {
			map = tcp_ext_to_lan;
		} else if (addressType == AT_UDP) {
			map = udp_ext_to_lan;
		} else {
			map = udp2_ext_to_lan;
		}
		...
		return ((InetSocketAddress)map.get(externalAddress));
	}
	...
	public void setIncludeWellKnownLANs(
		boolean	include) {
	...
	protected boolean addInstanceSupport(
		InetAddress			explicit_address,
