...
public class NonBlockingReadWriteService {
	...
	public NonBlockingReadWriteService(String _service_name, int timeout, int close_delay, ServiceListener _listener) {
		...
		readSelector = new VirtualChannelSelector(service_name, VirtualChannelSelector.OP_READ, false);
		writeSelector = new VirtualChannelSelector(service_name, VirtualChannelSelector.OP_WRITE, true);
			...
			while (true) {
				...
				boolean	stop_after_select = destroyed;
					...
					if (stop_after_select) {
						readSelector.destroy();
						writeSelector.destroy();
					}

				try {
				readSelector.select(50);
	...
	public void removeClientConnection(ClientConnection connection) {
		readSelector.cancel(connection.getSocketChannel());
		writeSelector.cancel(connection.getSocketChannel());
	...
	private void registerForSelection(final ClientConnection client) {
		...
		//WRITES
		final VirtualChannelSelector.VirtualSelectorListener writeListener = new VirtualChannelSelector.VirtualSelectorListener() {
			public boolean selectSuccess(VirtualChannelSelector selector, SocketChannel sc, Object attachment) {
				try {
					boolean more_writes_needed = client.writeMessages();
	
					if (more_writes_needed) {
						writeSelector.resumeSelects(client.getSocketChannel());	//we need to resume since write selects are auto-paused after select op
					}
		...
		writeSelector.register(client.getSocketChannel(), writeListener, null);	//start writing back to the connection
		writeSelector.pauseSelects(client.getSocketChannel());	 //wait until we've got something to send before selecting
	
		readSelector.register(client.getSocketChannel(), read_listener, null);	//start reading from the connection
	}
	...
	public void sendMessage(ClientMessage message) {
		...
		writeSelector.resumeSelects(vconn.getSocketChannel());	//start write selecting now that there's something to send
	}
