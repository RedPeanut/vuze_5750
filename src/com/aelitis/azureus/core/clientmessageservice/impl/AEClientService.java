...
public class AEClientService implements ClientMessageService {
	...
	private void connect() throws IOException {
		InetSocketAddress	tcpTarget = new InetSocketAddress(address, port);
		ConnectionEndpoint	ce = new ConnectionEndpoint(tcpTarget);
		ProtocolEndpointFactory.createEndpoint(ProtocolEndpoint.PROTOCOL_TCP, ce, tcpTarget);
		final AESemaphore connect_block = new AESemaphore("AEClientService:C");
		ce.connectOutbound(
			false,	// boolean		connectWithCrypto,
			false,	// boolean 		allowFallback,
			null,	// byte[][]		sharedSecrets,
			null,	// ByteBuffer	initialData,
			ProtocolEndpoint.CONNECT_PRIORITY_MEDIUM,	// int	priority,
			new Transport.ConnectListener() {	//NOTE: async operation!
