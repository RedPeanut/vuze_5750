	...
	implements ExternalSeedReader, PeerManagerListener2
{
	...
	protected ExternalSeedReaderImpl(
	...
	protected boolean readyToDeactivate(
		PeerManager	peer_manager,
		Peer		peer) {
		try {
				// obvious stuff first
	...
	protected void setActiveSupport(
		PeerManager	_peer_manager,
		boolean		_active) {
	...
	public void calculatePriorityOffsets(
		PeerManager		peer_manager,
		int[]			base_priorities) {
		try {
	...
	protected void informCancelled(
		PeerReadRequest		request) {
		for (int i=0;i<listeners.size();i++) {
