	...
	implements Plugin, DownloadManagerListener, DownloadListener, DownloadTrackerListener
{
	...
	public void initialize(
		PluginInterface	_pi) {
		pluginInterface	= _pi;
		logger				= pluginInterface.getLogger().getChannel("RunEverythingSeedingRules");
		...
		downloads_mon = pluginInterface.getUtilities().getMonitor();
		work_sem	= pluginInterface.getUtilities().getSemaphore();
		pluginInterface.getDownloadManager().addListener(this);
		pluginInterface.getUtilities().createTimer("DownloadRules", true).addPeriodicEvent(
			10000,
