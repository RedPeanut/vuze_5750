...
public class SWTSkin {
	...
	private static String TAG = SWTSkin.class.getSimpleName();
	...
	private void init(SWTSkinProperties skinProperties, boolean isDefault) {
			...
			SWTSkinObject[] existingObjects = arrayMap.get(key);
	  		...
	  		if (existingObjects != null) {
	  			...
	  			if (!bAlreadyPresent) {
	  				...
	  				//System.out.println("addToArrayMap: " + key + " : " + object + " #" + (length + 1));
	  			}
	  		} else {
	...
	*/
	...
	public void initialize(final Composite skincomp, String startID,
		...
		*/
		...
		Log.d(TAG, "sMainGroups.length = " + sMainGroups.length);
		for (int i = 0; i < sMainGroups.length; i++) {
			String sID = sMainGroups[i];
			Log.d(TAG, String.format("sID[%d] = %s", i, sID));
