...
{
	private static final String TAG = MainWindowImpl.class.getSimpleName();
	...
	protected MainWindowImpl(
		...
		gm.addListener(new GlobalManagerListener() {
			...
			public void downloadManagerAdded(final DownloadManager dm) {
				downloadAdded(new DownloadManager[] { dm });
			}
	...
	private void createWindow(IUIIntializer uiInitializer) {
		...
		try {
			...
			System.out.println("skin init took "
					+ (SystemTime.getCurrentTime() - startTime) + "ms");
			startTime = SystemTime.getCurrentTime();
			//Log.d(TAG, ">>> Utils.isAZ2UI() = " + Utils.isAZ2UI());
			if (Utils.isAZ2UI()) {
	...
	private void
	addTorrentsFromClipboard(String		text) {
