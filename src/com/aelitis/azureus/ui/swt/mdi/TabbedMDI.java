...
public class TabbedMDI
	...
	implements TabbedMdiInterface, 
		...
		ObfusticateImage
{
	private static String TAG = TabbedMDI.class.getSimpleName();
	...
	public TabbedMDI(Composite parent, String id) {
		...
		/*
		Log.d(TAG, "ctor(Composite parent, String id) is called...");
	...
	private void creatMDI() {
		if (soMain instanceof SWTSkinObjectTabFolder) {
			...
			tabFolder = new CTabFolder((Composite) soMain.getControl(),
				SWT.TOP | SWT.BORDER | SWT.CLOSE);
		}
		...
		if (minimizeVisible) {
			boolean toMinimize = ConfigurationManager.getInstance().getBooleanParameter(propsPrefix + ".subViews.minimized");
			setMinimized(toMinimize);
		}
	}

	private String getViewTitleID(
		String	view_id) {
	...
	private void minimize() {
		...
		configMan.setParameter(propsPrefix + ".subViews.minimized", true);
	}
	...
	private void restore() {
		...
		ConfigurationManager configMan = ConfigurationManager.getInstance();
		configMan.setParameter(propsPrefix + ".subViews.minimized", false);
	}
