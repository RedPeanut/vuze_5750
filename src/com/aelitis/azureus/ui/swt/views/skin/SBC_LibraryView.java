	...
	extends SkinView implements UIPluginViewToolBarListener
{
	...
	private static boolean	headerShowUptime;
	private static boolean	headerShowRates;

	private static volatile OverallStats 	totalStats;

	private static volatile int					selectionCount;
	private static volatile long				selectionSize;
	private static volatile long				selectionDone;
	private static volatile DownloadManager[]	selectionDms = {};
	...
	static {
		...
		COConfigurationManager.addAndFireParameterListeners(
			...
			new ParameterListener() {
				...
				private TimerEventPeriodic rateEvent;
				
				public void parameterChanged(String name) {
					
					headerShowUptime	= COConfigurationManager.getBooleanParameter("MyTorrentsView.showuptime");
					headerShowRates		= COConfigurationManager.getBooleanParameter("MyTorrentsView.showrates");
					SB_Transfers.triggerCountRefreshListeners();
					synchronized(this) {
						if (headerShowRates) {
							if (rateEvent == null) {
								rateEvent = SimpleTimer.addPeriodicEvent(
									"SBLV:rate-updater",
									1*1000,
									new TimerEventPerformer() {
										public void perform(
		...
		SelectedContentManager.addCurrentlySelectedContentListener(
			new SelectedContentListener() {
				public void currentlySelectedContentChanged(
					...
					String 				viewId ) {
					selectionCount = currentContent.length;
					long	total_size 	= 0;
					long	total_done	= 0;
					...
					selectionDms	= dms.toArray(new DownloadManager[ dms.size()]);
					SB_Transfers.triggerCountRefreshListeners();
				}
			});
	}
			...
			if (soLibraryInfo != null) {
				SB_Transfers.addCountRefreshListener(
					new SB_Transfers.countRefreshListener() {
						...
						{
							soLibraryInfo.getControl().getParent().setData(
								...
								new ViewUtils.ViewTitleExtraInfo() {
									public void update(
										...
										if (provider.value != count || provider.active != active) {
											...
											if (viewMode == provider.viewMode && provider.enabled) {
												SB_Transfers.triggerCountRefreshListeners();
											}
										}
									}
						...
						public void countRefreshed(
							...
							if (selectionCount > 1) {
								s += ", " +
										MessageText.getString(
										"label.num_selected", new String[]{ String.valueOf(selectionCount)});
								String	size_str = null;
								String	rate_str = null;
								if (selectionSize > 0) {
									if (selectionSize == selectionDone) {
										size_str = DisplayFormatters.formatByteCountToKiBEtc(selectionSize);
									} else {
										size_str = DisplayFormatters.formatByteCountToKiBEtc(selectionDone ) + "/" + DisplayFormatters.formatByteCountToKiBEtc( selectionSize);
									}
								}
								DownloadManager[] dms = selectionDms;
								if (headerShowRates && dms.length > 1) {
							...
							if (headerShowUptime && totalStats != null) {
						...
						class ExtraInfoProvider {
							...
							private ExtraInfoProvider(int viewMode) {
								this.viewMode = viewMode;
							}
						}
					}
