	...
	implements UPnPRootDevice
{
	...
	public NatPMPUPnPRootDeviceImpl(
		UPnP			_upnp,
		...
		throws Exception
	{
		...
		threadPool = new ThreadPool("NatPMPUPnP", 1, true);
	}
	...
	{
		private NatPMPImpl	natImpl;

		protected NatPMPUPnPWANConnection()

			throws UPnPException
		{
			natImpl	= new NatPMPImpl(nat_device);
		}
			...
			threadPool.run(
				new AERunnable() {
					public void runSupport() {
						try {
							natImpl.addPortMapping(tcp, port, description);
		...
		public UPnPWANConnectionPortMapping[] getPortMappings()
			throws UPnPException
		{
			return (natImpl.getPortMappings());
		}
			...
			throws UPnPException
		{
			threadPool.run(
				new AERunnable() {
					public void runSupport() {
						try {
							natImpl.deletePortMapping(tcp, port);
						} catch (UPnPException e) {
							e.printStackTrace();
						}
					}
		...
		public String getExternalIPAddress()
			...
			throws UPnPException
		{
			return ( natImpl.getExternalIPAddress());
		}

		public void periodicallyRecheckMappings(
			boolean	on) {
		}
